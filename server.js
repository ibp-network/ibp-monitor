...
;(async () => {
  ...
  if (process.env.P2P_PUBLIC_IP && process.env.P2P_PUBLIC_IP.length > 0) {
    if (isIPv4(process.env.P2P_PUBLIC_IP)) {
      const multiaddress = `/ip4/${process.env.P2P_PUBLIC_IP}/tcp/${config.listenPort}/p2p/${peerId}`
      ...
      const multiaddress = `/ip6/${process.env.P2P_PUBLIC_IP}/tcp/${config.listenPort}/p2p/${peerId}`
  ...
  if (process.env.P2P_PUBLIC_HOST && process.env.P2P_PUBLIC_HOST.length > 0) {
    if (isValidHostname(process.env.P2P_PUBLIC_HOST)) {
      const multiaddress = `/dnsaddr/${process.env.P2P_PUBLIC_HOST}/tcp/${config.listenPort}/p2p/${peerId}`
