    ...
    */
    ...
    this.app.get('/api/service/:serviceId', async (req, res) => {
      ...
      if (!service) {
        ...
        healthChecks.forEach((check) => (check.value = this._toJson(check.value)))
        let data = {
          version: this.version,
          localMonitorId: this.localMonitorId,
          dateTimeFormat: this.dateTimeFormat,
    ...
    this.app.get('/api/endpoint', async (req, res) => {
      ...
      let models = await this._ds.Service.findAll({
        include: 'monitors',
        order: [['name', 'ASC']],
      })
      let data = {
    ...
    this.app.get('/api/endpoint/:endpointUrl', async (req, res) => {
      ...
      const endpoint = await this._ds.Service.findByPk(endpointUrl, {
        include: ['monitors', 'peers'],
      })
