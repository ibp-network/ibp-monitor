  ...
  async handleMessage(evt) {
    ...
    switch (evt.detail.topic) {
      ...
      case '/ibp/healthCheck':
        ...
        console.log('/ibp/healthCheck from', shortStash(monitorId), 'for', serviceId, peerId)
        ...
        if (!memberServiceNode) {
          ...
          if (memberService) {
            if (!peerId) {
              ...
              await this._ds.HealthCheck.create(model)
              console.log('Created health check for', memberId, serviceId, shortStash(peerId))
            }
          } else {
