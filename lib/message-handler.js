  ...
  async handleMessage(evt) {
    ...
    switch (evt.detail.topic) {
      ...
      case '/ibp/healthCheck':
        const { memberId, serviceId, peerId, checkOrigin } = record
        model = {
          ...record,
          monitorId,
          ...
          checkOrigin: checkOrigin ?? 'member',
          source: 'gossip',
          // record
        }
        ...
        if (!memberServiceNode) {
          ...
          if (memberService) {
            if (!peerId) {
              logger.log('New member service node has null stash. Ignore.')
            } else {
              logger.log('New member service node:', memberId, serviceId, peerId)
              let node = {
                peerId,
                serviceId,
                memberId,
            ...
            logger.error('Member service not found:', memberId, serviceId)
            break
          }
        }
        const hc = await this._ds.HealthCheck.create(model)
        logger.log(`Created health check ${hc.id} for`, memberId, serviceId, shortStash(peerId))
        break
      case '/ibp/rpc':
        break
      default:
