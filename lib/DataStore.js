...
class DataStore {
  ...
  constructor(config = {}) {
    ...
    const Domain = sequelize.define('domain', domainModel.definition, { ...domainModel.options, sequelize })
    ...
    const Endpoint = sequelize.define('endpoint', endpointModel.definition, { ...endpointModel.options, sequelize })
    ...
    // Service.belongsTo(Peer, { as: 'peers', foreignKey: 'serviceUrl', otherKey: 'peerId', onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    Service.hasMany(HealthCheck, { as: 'healthChecks', foreignKey: 'serviceId', onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    // Service.hasOne(Member, { foreignKey: 'memberId' })
    // Service.belongsToMany(Monitor, { as: 'monitors', through: 'monitor_service', foreignKey: 'serviceUrl', otherKey: 'monitorId' })

    // Endpoint.belongsTo(Peer, { as: 'peers', foreignKey: 'serviceUrl', otherKey: 'peerId', onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    // Endpoint.hasMany(HealthCheck, { as: 'healthChecks', foreignKey: 'serviceUrl', onDelete: 'CASCADE', onUpdate: 'CASCADE' })
    // Endpoint.hasOne(Member, { foreignKey: 'memberId' })
    // Endpoint.belongsToMany(Monitor, { as: 'monitors', through: 'monitor_service', foreignKey: 'serviceUrl', otherKey: 'monitorId' })
  ...
  async sync () {
    ...
    await this.Member.sync({ alter: true })
