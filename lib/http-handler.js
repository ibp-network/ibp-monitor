...
import { HOUR_AS_MILLISECONDS } from './consts.js'
...
class HttpHandler {
  /** @type {import('../data/data-store.js').DataStore} */ ds
  ...
  setup() {
    ...
    this.app.get('/api/home', async (req, res) => {
      let monitorCount = await this.ds.Monitor.count()
      let memberCount = await this.ds.Member.count()
      let serviceCount = await this.ds.ProviderService.count()
      let checkCount = await this.ds.HealthCheck.count()
      let config = cfg
      config.sequelize = {}
      let data = {
    ...
    // members
    this.app.get('/api/member', async (_, res) => {
      console.debug('/api/member')
      let members = await this.ds.Member.findAll({
        include: {
          association: 'provider',
          order: [['name', 'ASC']],
        },
      }).then((members) =>
        members.map(({ provider, ...member }) => ({
          ...member.dataValues,
          ...provider.dataValues,
        }))
      )
    ...
    // single member
    this.app.get('/api/member/:providerId', async ({ params: { providerId } }, res) => {
      console.debug(`app.get(/api/member/${providerId})`)

      const {
        provider: { dataValues: provider },
        dataValues: member,
      } = await this.ds.Member.findOne({
        where: { id: providerId },
        include: ['membershipLevel', 'provider'],
      })
      const memberServices = await this.ds.ProviderService.findAll({
        where: { providerId },
      })
      ...
      if (!member) {
        ...
        const services = await this.ds.Service.findAll({
    ...
    this.app.get('/api/member/:providerId/healthChecks', async (req, res) => {
      let { providerId } = req.params
      console.debug(`app.get(/api/member/${providerId}/healthChecks)`)
      const healthChecks = await this.ds.HealthCheck.findAll({
        where: { providerId },
    ...
    // single member nodes
    this.app.get('/api/member/:providerId/nodes', async ({ params: { providerId } }, res) => {
      console.debug(`app.get(/api/member/${providerId}/nodes)`)
      const nodes = await this.ds.ProviderServiceNode.findAll({
        where: { providerId },
        order: [['serviceId', 'ASC']],
      })
      res.json({ nodes })
    ...
    // services
    this.app.get('/api/service', async (_, res) => {
      console.debug('/api/service')

      let services = await this.ds.Service.findAll({
        order: [['id', 'ASC']],
    ...
    // single service
    this.app.get('/api/service/:serviceId', async ({ params: { serviceId } }, res) => {
      console.debug(`app.get(/api/service/${serviceId})`)

      const service = await this.ds.Service.findOne({
    ...
    this.app.get('/api/monitor', async (req, res) => {
      ...
      let monitors = await this.ds.Monitor.findAll()
      let data = {
        version: this.version,
        localMonitorId: this.localMonitorId,
        dateTimeFormat: this.dateTimeFormat,
    ...
    this.app.get('/api/monitor/:id', async (req, res) => {
      ...
      let { id } = req.params
      const monitor = await this.ds.Monitor.findByPk(id)
      if (!monitor) {
    ...
    this.app.get('/api/healthCheck', async (req, res) => {
      ...
      let isMember = (req.query.isMember || 'true') === 'true'
      let offset = Number(req.query.offset) || 0
      let limit = Number(req.query.limit) || 15

      console.log({ isMember, offset, limit })
      let count = await this.ds.HealthCheck.count()
      let models = await this.ds.HealthCheck.findAll({
        where: { memberId: isMember ? { [Op.ne]: null } : null },
        order: [['id', 'DESC']],
        limit,
        offset,
      })
    ...
    this.app.get('/api/healthCheck/:id', async (req, res) => {
      ...
      let { id } = req.params
      let { raw } = req.query
      let model = await this.ds.HealthCheck.findByPk(id)
      if (!model) {
        // res.send(this._notFound())
        res.json({ error: 'not found' })
      } else {
    ...
    this.app.get('/api/geoDnsPool', async (req, res) => {
      ...
      let geoDnsPools = await this.ds.GeoDnsPool.findAll()
      let data = {
        version: this.version,
        localMonitorId: this.localMonitorId,
        dateTimeFormat: this.dateTimeFormat,
    ...
    this.app.get('/api/status', async (req, res) => {
      ...
      let members = await this.ds.Member.findAll({ include: ['provider'] }).then((members) =>
        members.map(({ dataValues: member, provider: { dataValues: provider } }) => ({
          ...provider,
          ...member,
        }))
      )
      let status = {}
      let startDate = new Date(new Date().getTime() - 47 * HOUR_AS_MILLISECONDS)
      startDate.setMinutes(0)
      startDate.setSeconds(0)
      startDate.setMilliseconds(0)
      let healthChecks = await this.ds.HealthCheck.findAll({
        where: {
          createdAt: {
            [Op.gte]: startDate,
          },
      ...
      for (let healthCheck of healthChecks) {
        ...
        if (serviceStatus) {
          ...
          if (hourStatus) {
            let memberStatus = hourStatus[healthCheck.providerId]
            if (!memberStatus) {
              status[healthCheck.serviceId][hourTimestamp][healthCheck.providerId] = {
                success: 0,
                warning: 0,
                error: 0,
              }
        ...
        let value = status[healthCheck.serviceId][hourTimestamp][healthCheck.providerId]
        ...
        status[healthCheck.serviceId][hourTimestamp][healthCheck.providerId] = value
      }
      res.json({
        services,
        members,
