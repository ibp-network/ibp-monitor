...
class PrometheusExporter {
  ...
  _calcAverage (checks=[], range=10) {
    ...
    if (count > 0) {
      var sum = chunk
        .map(m => { return this._parseRecord(m.record).performance || 0} )
        .reduce((prevVal, currVal, idx, arr) => { return prevVal + currVal }, 0)
      ret = sum / count  
    }
    return ret
  }
  ...
  async export (serviceUrl) {
    ...
    if (!service) {
      ...
      lines.push(`ibp_service_performance_1{serviceUrl="${serviceUrl}", timestamp="${moment.utc(check.createdAt).valueOf()}"} ${record?.performance || 0}`)
