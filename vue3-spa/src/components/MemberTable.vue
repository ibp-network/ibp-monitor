<template>
  ...
  <table class="table is-fullwidth">
    <thead>
      ...
      <th v-if="columns.includes('services')" class="text-centered"><v-icon size="small">mdi-server</v-icon>Services</th>
      <th v-if="columns.includes('membership')">Membership</th>
      <th v-if="columns.includes('current_level')"><v-icon size="small">mdi-seal-variant</v-icon>Level</th>
    ...
    <tbody>
      <tr v-for="member in members" v-bind:key="member.memberId"  style="cursor: pointer" @click="gotoMember(member.memberId)">
        ...
        <td v-if="columns.includes('services')" class="text-center">{{member.services?.length || 0}}</td>
        <td v-if="columns.includes('membership')">{{member.membership}}</td>
        <td v-if="columns.includes('current_level')" class="text-center">{{member.current_level}}</td>
